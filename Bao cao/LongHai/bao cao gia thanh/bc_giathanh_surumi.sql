...
FROM (
    ...
    FROM (
        ...
        FROM fact_acct fc
        JOIN c_elementvalue acct ON fc.account_id = acct.c_elementvalue_id AND LEFT(acct.value, 2) IN ('15')
        WHERE fc.ad_client_id = $P{AD_CLIENT_ID} AND fc.C_Activity_ID = 1000273 AND ad_table_id = 325 AND M_Product_ID = 1015097 AND fc.dateacct>=$P{StartDate} AND fc.dateacct<=$P{EndDate} 
        UNION ALL
...
UNION ALL
SELECT 'II' AS nhom,k.description, k.name,k.hsv_dinhmuc,k.id,round(k.qty,0) AS qty,round(k.tt,0) AS tt,  round(k.dg,0) AS dg FROM
(
SELECT tp.description, tp.name, tp.hsv_dinhmuc, tp.value AS id, NULL::int4 AS qty, NULL::int4 AS tt, NULL::int4 AS dg
FROM hsv_giathanhvan tp
...
FROM(
	...
	SELECT '7' AS id, 0 AS qty, sum(-fc.amtacctcr + fc.amtacctdr) AS "tt"
        ...
        WHERE fc.ad_client_id = $P{AD_CLIENT_ID} AND fc.C_Activity_ID = 1000273 AND ad_table_id = 321 AND  fc.C_Campaign_ID=1000062  AND fc.dateacct>=$P{StartDate} AND fc.dateacct<=$P{EndDate} 
) AS k GROUP BY id
) AS k 
RIGHT JOIN hsv_giathanhvan tp ON tp.value = k.id
...
UNION ALL
	...
	WHERE fc.ad_client_id = $P{AD_CLIENT_ID} AND fc.C_Activity_ID = 1000273  AND fc.dateacct>=$P{StartDate} AND fc.dateacct<=$P{EndDate} 
	GROUP BY id
...
FROM (
	...
	JOIN m_productionplan  civ ON ci.m_production_id = civ.m_production_id 
	JOIN m_product hh ON civ.m_product_id = hh.m_product_id AND hh.M_AttributeSet_ID=1000088 
	WHERE ci.ad_client_id = $P{AD_CLIENT_ID} AND ci.docstatus IN ('CO','CL') AND ci.c_activity_id = 1000273 AND ci.c_campaign_id=1000062  AND ci.movementdate>=$P{StartDate} AND ci.movementdate<=$P{EndDate} 
