...
FROM (
	...
	WHERE cil.ad_client_id = 1000017 AND ci.dateacct >= '10-01-2018' AND ci.dateacct <= '12-31-2018' AND ci.issotrx = 'N'
	...
	JOIN c_invoice ci ON ci.c_invoice_id = cil.c_invoice_id AND ci.docstatus IN ('CO', 'CL')
	JOIN m_product hh ON hh.m_product_id = cil.m_product_id AND hh.m_product_category_id IN (1000450) AND hh.M_AttributeSet_ID = 1000067
	WHERE cil.ad_client_id = 1000017 AND ci.dateacct >= '01-01-2018' AND ci.dateacct <= '12-31-2018' AND ci.issotrx = 'N'
	) AS t
GROUP BY t.id
...
FROM (
	...
	FROM (
		SELECT '7' AS id, sum(k.sl) AS b1, 0 AS b2 --soluong van boc duoc theo lo--
		FROM (
			SELECT hh.m_product_id, att.m_attributesetinstance_id, att.lot, sum(civ.qtyinvoiced) AS sl, sum(civ.linetotalamt) AS tt
			FROM c_invoiceline civ
			JOIN c_invoice ci ON ci.c_invoice_id = civ.c_invoice_id AND ci.docstatus IN ('CO', 'CL')
...
FROM (
	...
	WHERE cil.ad_client_id = 1000017 AND ci.issotrx = 'Y' AND ci.dateacct >= '10-01-2018' AND ci.dateacct <= '12-31-2018'
	...
	JOIN c_invoice ci ON ci.c_invoice_id = cil.c_invoice_id AND ci.docstatus IN ('CO', 'CL')
	JOIN m_product hh ON hh.m_product_id = cil.m_product_id AND hh.M_Product_ID = 1012371
	WHERE cil.ad_client_id = 1000017 AND ci.issotrx = 'Y' AND ci.dateacct >= '01-01-2018' AND ci.dateacct <= '12-31-2018'
	) AS t
GROUP BY t.id
...
FROM (
	...
	JOIN m_lot lo ON lo.name = t.lot AND lo.dateto >= '10-01-2018' AND lo.dateto <= '12-31-2018' AND lo.m_lot_id IS NOT NULL
	...
	JOIN m_lot lo ON lo.name = t.lot AND lo.dateto >= '01-01-2018' AND lo.dateto <= '12-31-2018' AND lo.m_lot_id IS NOT NULL
	) AS t
GROUP BY t.id
...
FROM (
	...
	FROM (
		SELECT hh.m_product_id,  sum(civ.qtyinvoiced) AS sl, sum(civ.linenetamt) AS tt
		FROM c_invoiceline civ
		JOIN c_invoice ci ON ci.c_invoice_id = civ.c_invoice_id AND ci.docstatus IN ('CO', 'CL')
...
UNION ALL
--10--
SELECT 'KQ thu từ bóc  (6+7-7.1+8+9)' AS a1, 'đ' AS a2, sum(a3) AS a3, sum(a4) AS a4, id
FROM (
	SELECT '14' AS id, (sum(b6) + sum(b7) - sum("b7.1") + sum(b8) + sum(b9)) AS a3, 0 AS a4
	FROM (
		---6--
		SELECT '14' AS id, (sum(k.b1) + sum(k.b2) + sum(k.b3)) - sum(b4) AS b6, 0 AS b7, 0 AS "b7.1", 0 AS b8, 0 AS b9
		FROM (
			SELECT '9' AS id, sum(c1) * 2850000 AS b1, 0 AS b2, 0 AS b3, 0 AS b4
			FROM (
				SELECT sum(civ.productionqty) AS c1
		...
		--7--        
		SELECT '14' AS id, 0 AS b6, sum(cil.linetotalamt) AS b7, 0 AS "b7.1", 0 AS b8, 0 AS b9
		FROM c_invoiceline cil
		JOIN c_invoice ci ON ci.c_invoice_id = cil.c_invoice_id AND ci.docstatus IN ('CO', 'CL')
		JOIN m_product hh ON hh.m_product_id = cil.m_product_id AND hh.M_Product_ID = 1012371
		WHERE cil.ad_client_id = 1000017 AND ci.issotrx = 'Y' AND ci.dateacct >= '10-01-2018' AND ci.dateacct <= '12-31-2018'
		...
		--7.1----
		SELECT '14' AS id, 0 AS b6, 0 AS b7, sum(t.tt) AS "b7.1", 0 AS b8, 0 AS b9
		FROM (
			SELECT hh.m_product_id, att.lot, sum(civ.qtyinvoiced) AS sl, sum(civ.linetotalamt) AS tt
			FROM c_invoiceline civ
			JOIN c_invoice ci ON ci.c_invoice_id = civ.c_invoice_id AND ci.docstatus IN ('CO', 'CL')
		...
		--- b8----
		SELECT '14' AS id, 0 AS b6, 0 AS b7, 0 AS "b7.1", sum(t.tt) AS b8, 0 AS b9
		FROM (
			SELECT hh.m_product_id, sum(civ.qtyinvoiced) AS sl, sum(civ.linetotalamt) AS tt
			FROM c_invoiceline civ
			JOIN c_invoice ci ON ci.c_invoice_id = civ.c_invoice_id AND ci.docstatus IN ('CO', 'CL')
			JOIN m_product hh ON civ.m_product_id = hh.m_product_id AND hh.M_Product_ID = 1012372
			WHERE civ.ad_client_id = 1000017 AND ci.issotrx = 'Y' AND ci.dateacct >= '10-01-2018' AND ci.dateacct <= '12-31-2018'
		...
		SELECT '14' AS id, 0 AS b6, 0 AS b7, 0 AS "b7.1", 0 AS b8, sum(t.tt) AS b9
		FROM (
			SELECT hh.m_product_id, sum(civ.qtyinvoiced) AS sl, sum(civ.linetotalamt) AS tt
			FROM c_invoiceline civ
			JOIN c_invoice ci ON ci.c_invoice_id = civ.c_invoice_id AND ci.docstatus IN ('CO', 'CL')
	...
	--------############################-------
	SELECT '14' AS id, 0 AS a3, (sum(b6) + sum(b7) - sum("b7.1") + sum(b8) + sum(b9)) AS a4
	FROM (
		---6--
		SELECT '14' AS id, (sum(k.b1) + sum(k.b2) + sum(k.b3)) - sum(b4) AS b6, 0 AS b7, 0 AS "b7.1", 0 AS b8, 0 AS b9
		FROM (
