...
FROM (
	...
	WHERE civ.ad_client_id = 1000017 AND ci.dateacct >= '01-10-2018' AND ci.dateacct <= '12-31-2018' AND ci.issotrx = 'N'
...
FROM (
	...
	JOIN m_lot lo ON lo.name = k.lot AND lo.dateto >= '01-10-2018' AND lo.dateto <= '12-31-2018' AND lo.m_lot_id IS NOT NULL
...
FROM (
	...
	JOIN m_lot lo ON lo.name = tt.lot AND lo.dateto >= '01-10-2018' AND lo.dateto <= '12-31-2018' AND lo.m_lot_id IS NOT NULL
...
--2/3-- phat sinh
    ...
    --2--
        ...
        FROM (
            ...
            JOIN m_product hh ON civ.m_product_id = hh.m_product_id AND hh.M_Product_Category_ID = 1000450 AND hh.M_Product_ID <> 1013024
            JOIN m_attributesetinstance att ON civ.m_attributesetinstance_id = att.m_attributesetinstance_id AND att.M_AttributeSet_ID = 1000067
            JOIN m_lot lo ON lo.name = att.lot AND lo.dateto >= '01-10-2018' AND lo.dateto <= '12-31-2018' AND lo.m_lot_id IS NOT NULL
            WHERE civ.ad_client_id = 1000017 AND ci.issotrx = 'N'
            GROUP BY att.m_attributesetinstance_id, hh.m_product_id
		) AS k
        --3---
...
FROM(
    ...
    WHERE civ.ad_client_id = 1000017 AND ci.dateacct >= '01-10-2018' AND ci.dateacct <= '12-31-2018' AND ci.issotrx = 'Y'
...
FROM (
    ...
    FROM(   
        ...
        FROM c_invoiceline civ
        JOIN c_invoice ci ON ci.c_invoice_id=civ.c_invoice_id AND ci.docstatus IN ('CO','CL') AND civ.M_Product_ID=1012372  
        WHERE civ.ad_client_id=1000017 AND ci.issotrx ='Y' AND   ci.dateacct>='01-10-2018' AND ci.dateacct<='12-31-2018'
...
FROM (
    ...
    FROM(   
        SELECT sum(civ.qtyinvoiced) AS sl, sum(civ.linenetamt) AS tt 
        FROM c_invoiceline civ
        JOIN c_invoice ci ON ci.c_invoice_id=civ.c_invoice_id AND ci.docstatus IN ('CO','CL') AND civ.M_Product_ID=1012373  
        WHERE civ.ad_client_id=1000017 AND ci.issotrx ='Y' AND   ci.dateacct>='01-10-2018' AND ci.dateacct<='12-31-2018'
        GROUP BY civ.m_product_id 
    ) AS t    
    UNION ALL
